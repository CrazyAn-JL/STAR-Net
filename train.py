...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        loss = 0.5 * multiscale_ssim_loss(output_rgb, gt_rgb) + L1_loss(output_rgb, gt_rgb)  + E_loss(output_rgb,gt_rgb) + opt.P_weight * P_loss(output_rgb, gt_rgb)[0]
        iter += 1
...
def frequency_consistency_loss(output, gt):
    out_fft = torch.fft.fft2(output, norm='ortho')
    gt_fft = torch.fft.fft2(gt, norm='ortho')

    out_amp = torch.log(torch.abs(out_fft) + 1e-8)
    gt_amp = torch.log(torch.abs(gt_fft) + 1e-8)

    return F.l1_loss(out_amp, gt_amp)
